...
//        $this->usersID = '0';
    ...
    function getEquipment($user_id) {
        ...
        $whereU = 'AND userID IN('.$this->usersID.','.$user_id.')';
    ...
    function sortEq ($data, $user_id) {
        ...
        foreach ($columns as $column => $value) {
            ...
            $query = "SELECT id, {$column} FROM db_product WHERE {$column} IS NOT NULL {$where} {$whereU} GROUP BY {$column}";
    ...
    function items( $data, $user_id) {
        ...
        $whereU = ' userID IN('.$this->usersID.','.$user_id.')';
...
//        var_dump($product_data, $data->db_pro_PK); exit;
    ...
    function getProductAddInfo($db_pro_PK) {
        ...
        $product_data[0]['weight'] = array('value'=>$product_data[0]['weight'], 'unit'=>$product_data[0]['unit']['weight']['unit']);
...
//    }
    ...
    function getCalcEquipment($calcPK) {
        ...
        $res = array(
            ...
            'tia_222_g_wl_da_PK' => $data,
            'tia_222_g_wl_da_lib_PK' => $data_lib
        );
...
//        if($res){
        ...
        foreach ($data->faces as $key => $value) {
            ...
            (`name`,          `tia_222_g_wl_ida_PK`, `inclusion`, `face_name`,   `face_shape`,  `display`,  `face_azm`,  `exposed`,  `p_a`,  `aspect_ratio`,  `c_a`,  `epa_a`,  `notes`) 
            VALUES 
            ('{$data->name}', '{$equip_id_ida}',     '$inclusion', '$face_name', '$face_shape', '$display', '$face_azm', '$exposed', '$p_a', '$aspect_ratio', '$c_a', '$epa_a', '$notes')";
                        ...
                        if($loading->tia_222_g_wl_da_lib_PK == $eq_old_id) {
                            ...
                            if($status = $this->db->query($query_loading)) {
                                ...
                                foreach ($loading->faces as $face) {
                                    ...
                                    $query_face = "INSERT INTO `tia_222_g_wl_ida_faces` (`tia_222_g_wl_ida_PK`,  `name`,           `inclusion`,            `face_name`,              `face_shape`,              `display`,             `face_azm`,             `exposed`,            `p_a`,            `aspect_ratio`,            `c_a`,            `epa_a`,             `notes`) 
                                                                                 VALUES ('$ida_new_id',          '$face->name',    '$face->inclusion',     '$face->face_name',       '$face->face_shape',       '$face->display',      '$face->face_azm',      '$face->exposed',     '$face->p_a',     '$face->aspect_ratio',     '$face->c_a',     '$face->epa_a',      '$face->notes')";
...
switch ($method) {
    case "getProductsList":
        $result = $generic->getProductsList();
