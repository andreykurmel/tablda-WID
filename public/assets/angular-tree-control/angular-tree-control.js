    ...
    angular.module( 'treeControl', [] )
        ...
        .directive( 'treecontrol', ['$compile', function( $compile ) {
            ...
            return {
                ...
                controller: ['$scope', "$location", '$templateCache', '$interpolate', 'treeConfig', function( $scope, $location, $templateCache, $interpolate, treeConfig ) {
                    ...
                    $scope.headClass = function(node) {
                        ...
                        if (($scope.options.isLeaf(node) && node.type == "folder") ||( node.app && node.listUsername && node.listUsername.length == 0))
                            return "tree-empty" + injectSelectionClass;

                        if ($scope.options.isLeaf(node)) {
                            if (node.type == "link") {
                                return "tree-link" + injectSelectionClass;
                            }
                    ...
                    $scope.selectNodeLabel = function( selectedNode){
                        ...
                        var transcludedScope = this;
                        if(selectedNode.type == "folder" || !$scope.options.isLeaf(selectedNode) && (!$scope.options.dirSelectable || !$scope.options.isSelectable(selectedNode))) {
                    ...
                    $scope.premiumClass = function(){
                        return $scope.options.isPremier(this.node) ? "tree-premium " : "";
                    };

                    $scope.selectableClass = function () {
                        var style = '';
                        var state = $scope.options.isAccess(this.node);
                    ...
                    if(!template) {
                        template =
                            '<ul {{options.ulClass}} >' +
                            '<li ng-repeat="node in node.{{options.nodeChildren}} | filter:filterExpression:filterComparator {{options.orderBy}}" ng-class="[headClass(node), selectableClass()]" {{options.liClass}}' +
                            'set-node-to-data>' +
                            '<i class="tree-branch-head" ng-class="iBranchClass()" ng-click="selectNodeHead(node)"></i>' +
                            '<i class="tree-leaf-head {{options.iLeafClass}}"></i>' +
                            '<div class="tree-label {{options.labelClass}}" ng-class="[selectedClass(), unselectableClass(), premiumClass()]" ng-click="selectNodeLabel(node)" tree-transclude></div>' +
