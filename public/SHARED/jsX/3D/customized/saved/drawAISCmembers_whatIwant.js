function drawAISCmember(XD, approach, app_p1, app_p2, app_p3, dbSRC, Shape, SizeType, SizeUnit, Size1, Size2, material, rot_x2stdy, shrk_pct) {
    ...
    switch (Shape) {
        ...
        case '2L_SLBB':
            ...
            console.log('2L / shapeData: ' + shapeData);
            ...
            var nodes_part1 = [[-xbar, b - ybar],
                ...
                var dist = Math.pow(Math.pow(nodes_part1[inode][0] - nodes_part1[inode + 1][0], 2) + Math.pow(nodes_part1[inode][1] - nodes_part1[inode + 1][1], 2), 1 / 2);
                var node_mdl = [(nodes_part1[inode][0] + nodes_part1[inode + 1][0]) / 2, (nodes_part1[inode][1] + nodes_part1[inode + 1][1]) / 2];
                var node_ctrl_2 = [(round && round.symmetry ? node_mdl[0] + dist / nbr_div : node_mdl[0] - dist / nbr_div), node_mdl[1] + dist / nbr_div];

                var inode = 4;
                var dist = Math.pow(Math.pow(nodes_part1[inode][0] - nodes_part1[inode + 1][0], 2) + Math.pow(nodes_part1[inode][1] - nodes_part1[inode + 1][1], 2), 1 / 2);
                var node_mdl = [(nodes_part1[inode][0] + nodes_part1[inode + 1][0]) / 2, (nodes_part1[inode][1] + nodes_part1[inode + 1][1]) / 2];
                var node_ctrl_3 = [(  node_mdl[0] + dist / nbr_div), node_mdl[1] - dist / nbr_div];

                var nodes_ctrl_part1 = [node_ctrl_1, node_ctrl_2, node_ctrl_3];
            ...
            for (var i = 0; i < nodes_ctrl_part1.length; i++) {
                for (var j = 0; j < 1; j++) {
                    switch (Shape) {
                        case '2L_E':
                            nodes_ctrl_part1[i][j] = nodes_ctrl_part1[i][j] + xbar + spacing / 2;
                            break;
                        case '2L_LLBB':
                            nodes_ctrl_part1[i][j] = nodes_ctrl_part1[i][j] + xbar + spacing / 2;
                            break;
            ...
            for (var i = 0; i < nodes_part2.length; i++) {
                for (var j = 0; j < 1; j++) {
                    nodes_part2[i][j] = (-1) * nodes_part2[i][j];
                }
            }

            console.log('nodes_part1, before: ' + nodes_part1[0][0]);
            console.log('nodes_part2, before: ' + nodes_part2[0][0]);

            var nodes = [crdtTrft(nodes_part1), crdtTrft(nodes_part2)];
            // var nodes = [crdtTrft(nodes_part1)];
            // nodes_part1 = nodes[0];
            // nodes_part2 = nodes[1];
            // console.log('nodes_part1, after: ' + nodes_part1[0][0]);
            // console.log('nodes_part2, after: ' + nodes_part2[0][0]);

            var nodes_ctrl_part2 = nodes_ctrl_part1;
            for (var i = 0; i < nodes_ctrl_part2.length; i++) {
                for (var j = 0; j < 1; j++) {
                    nodes_ctrl_part2[i][j] = (-1) * nodes_ctrl_part2[i][j];
                }
            } 
            var nodes_ctrl = [crdtTrft(nodes_ctrl_part1), crdtTrft(nodes_ctrl_part2)];           
    ...
    // I want to treat symmetryNodes as a set of nodes for a regular part (iPart>=1) different from nodes[iPart=0].
    // so following code to get symmetryNodes is to be done in the code for each case above.
/*    var is2LE = Shape == '2L_E', symmetryNodes = [];
    ...
    if (XD == "2D" || XD == "23D") {
        ...
        drawAISC_2D(Shape, nodes, nodes_hole, nodes_ctrl, zoomscale);
    }
    ...
    if (XD == "3D" || XD == "23D") {
        ...
        // TODO 3D
        //mesh = new draw_3D(Shape, approach, app_p1, app_p2, app_p3, nodes, nodes_hole, nodes_ctrl, material);
...
/*        if (symmetryNodes.length) {
        ...
        //mesh = new draw_3D(Shape, approach, app_p1, app_p2, app_p3, nodes[0], nodes_hole, nodes_ctrl, material);
        //for (var iPart = 1; iPart < nodes.length; iPart++) {
        //    var mesh_add = new draw_3D(Shape, approach, app_p1, app_p2, app_p3, nodes[iPart], nodes_hole, material);
        //    THREE.GeometryUtils.merge(mesh.geometry, mesh_add.geometry);
        //};
        //
        //mesh.rot = rot_x2stdy;
    }
...
function draw_3D(Shape, approach, app_p1, app_p2, app_p3, nodes, nodes_hole, nodes_ctrl, material) {
    ...
    var txtOn = false;
    var crossSection = drawShapeBdry(Shape, nodes, nodes_hole, nodes_ctrl, ctx_ShapeBdry, path_Hole, txtOn);
...
function drawAISC_2D(Shape, nodes, nodes_hole, nodes_ctrl, zoomscale, symmetryNodes) {
    ...
    function drawAll() {
        ...
        ctx_ShapeBdry = drawShapeBdry(Shape, nodes, nodes_hole, nodes_ctrl, ctx_ShapeBdry, path_Hole, txtOn);
...
/*        if (symmetryNodes && symmetryNodes.length) {
        ...
        // I want to do something like this
        //for (var iPart = 1; iPart < nodes.length; iPart++) {
        //    ctx_ShapeBdry = drawShapeBdry(Shape, nodes[iPart], nodes_hole, nodes_ctrl, ctx_ShapeBdry, path_Hole, txtOn);
        //};
...
function drawShapeBdry(Shape, nodes, nodes_hole, nodes_ctrl, ctx_ShapeBdry, path_Hole, txtOn) {
    switch (Shape) {
        ...
        case "2L_E":
            ...
            console.log("NDSCRL", nodes_ctrl);

            ctx_ShapeBdry.quadraticCurveTo(nodes_ctrl[ipart][0][0], nodes_ctrl[ipart][0][1], nodes[ipart][inode + 1][0], nodes[ipart][inode + 1][1]);
...
/*                var inode = 2;
                ...
                */
                ctx_ShapeBdry.quadraticCurveTo(nodes_ctrl[ipart][1][0], nodes_ctrl[ipart][1][1], nodes[ipart][inode + 1][0], nodes[ipart][inode + 1][1]);
...
/*                var inode = 4;
                ...
                ctx_ShapeBdry.quadraticCurveTo(nodes_ctrl[ipart][2][0], nodes_ctrl[ipart][2][1], nodes[ipart][inode + 1][0], nodes[ipart][inode + 1][1]);
